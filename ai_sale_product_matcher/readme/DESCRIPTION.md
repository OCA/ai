...
Sales (Satış) integration: `sale.order` → *Find Products with AI* wizard (file + text → Extract → editable JSON → Find Matches → Add to Order). Extraction runs via `queue_job` + `bus` so UI never blocks.
