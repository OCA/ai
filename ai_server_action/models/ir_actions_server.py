...
class IrActionsServer(models.Model):
    ...
    def _run_action_ai_oca(self, eval_context=None):
        ...
        prompt = self._get_ai_oca_prompt(record)
        result = self.ai_connection_id._run(
            prompt,
            tools=self.ai_tool_ids,
            record=record,
        )[0]
        self._post_run_action_ai_oca(result, eval_context, record)

    def _post_run_action_ai_oca(self, result, eval_context, record):
        self.ensure_one()
