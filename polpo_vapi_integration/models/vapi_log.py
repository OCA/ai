...
class VapiLog(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="User", required=True, help="The user who made the call"
    )
    call_id = fields.Char(
        "Odoo Call ID", required=True, help="Unique identifier for the call"
    )
    start_time = fields.Datetime(
        "Start", required=True, help="Date and time when the call started"
    )
    end_time = fields.Datetime("End", help="Date and time when the call ended")
    duration = fields.Integer(
        "Duration (sec.)", help="Call duration in seconds, 0 if not ended"
    )
    state = fields.Selection(
        ...
        default="in_progress",
        required=True,
        help="The current status of the call. 'Started' when initiated,"
        " 'In Progress' during the call, "
    ...
    def cron_close_stuck_calls(self, minutes=10):
        """
        Closes calls that have been stuck for more than a
