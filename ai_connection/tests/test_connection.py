...
class TestConnection(TransactionCase):
    ...
    def test_execution_step_error_caught(self):
        ...
        with patch.object(
            type(connection),
            "_execute_ai_call",
            side_effect=ValueError("AI exploded!"),
        ):
            execution.action_start()
            self.assertEqual(execution.state, "failed")
    ...
    def test_execution_async_start_and_resume(self):
        connection = self.env["ai.connection"].create({"name": "C", "kind": "demo"})
        execution = self.env["ai.connection.execution"].create(
            {
                "connection_id": connection.id,
        ...
        with patch("odoo.addons.base.models.ir_cron.ir_cron._trigger") as mock_trigger:
            execution.action_start()
            self.assertEqual(execution.state, "running")
            self.assertEqual(execution.current_iteration, 0)
            mock_trigger.assert_called_once()

        execution.state = "pending_tool_approval"
        with patch("odoo.addons.base.models.ir_cron.ir_cron._trigger") as mock_trigger:
            execution.action_resume()
            self.assertEqual(execution.state, "running")
            self.assertEqual(execution.current_iteration, 0)
    ...
    def test_cron_process_single_step(self):
        connection = self.env["ai.connection"].create({"name": "C", "kind": "demo"})
        execution = self.env["ai.connection.execution"].create(
            {
                "connection_id": connection.id,
        ...
        # Test it processes one step and triggers again
        with (
            patch.object(type(execution), "_execute_step") as mock_step,
            patch("odoo.addons.base.models.ir_cron.ir_cron._trigger") as mock_trigger,
        ):
            # It will still be running after the mock returns
            self.env["ai.connection.execution"]._cron_process_single_step()

            mock_step.assert_called_once()
            mock_trigger.assert_called_once()

        # Test it halts if state is not running
        execution.state = "done"
        execution2 = self.env["ai.connection.execution"].create(
            {
                "connection_id": connection.id,
                "state": "running",
            }
        ...
        with (
            patch.object(type(execution2), "_execute_step") as mock_step,
            patch("odoo.addons.base.models.ir_cron.ir_cron._trigger") as mock_trigger,
        ):

            def side_effect():
                execution2.state = "done"
    ...
    @mute_logger("odoo.addons.ai_connection.models.ai_connection_execution")
    def test_cron_process_single_step_failure(self):
        connection = self.env["ai.connection"].create({"name": "C", "kind": "demo"})
        execution = self.env["ai.connection.execution"].create(
            {
                "connection_id": connection.id,
        ...
        with patch.object(
            type(connection),
            "_execute_ai_call",
            side_effect=ValueError("Async exploded!"),
        ):
            self.env["ai.connection.execution"]._cron_process_single_step()

            # _execute_step catches the exception and sets state to failed
            self.assertEqual(execution.state, "failed")
            self.assertEqual(execution.error_message, "Async exploded!")
